...
final class RealBufferedSource implements BufferedSource {
	...
	public int select(Options options) throws IOException {
		...
		while (true) {
			...
			// If the prefix match actually matched a full byte string, consume
			// it and return it.
	...
	public String readUtf8LineStrict(long limit) throws IOException {
		...
		throw new EOFException("\\n not found: limit="
				+ Math.min(buffer.size(), limit) + " content="
	...
	public long indexOf(byte b, long fromIndex, long toIndex)
		...
		while (fromIndex < toIndex) {
			...
			if (lastBufferSize >= toIndex
					|| source.read(buffer, Segment.SIZE) == -1)
				return -1L;
